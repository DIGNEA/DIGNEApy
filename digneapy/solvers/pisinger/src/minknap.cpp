...
extern state *findvect(stype ws, state *f, state *l) {
...
extern void push(allinfo *a, int side, item *f, item *l) {
    interval *pos;
    switch (side) {
        case LEFT:
            pos = a->intv1;
...
extern void pop(allinfo *a, int side, item **f, item **l) {
    interval *pos;
    switch (side) {
        case LEFT:
...
extern void improvesolution(allinfo *a, state *v) {
...
extern void definesolution(allinfo *a) {
    register item *f, *l, *i;
...
extern item *median(item *f1, item *l1, ntype s) {
...
extern void partsort(allinfo *a, item *f, item *l, stype ws, int what) {
    register ptype mp, mw;
    register item *i, *j, *m;
    register stype wi;
    register int d;
...
extern boolean haschance(allinfo *a, item *i, int side) {
    register state *j, *m;
...
extern void multiply(allinfo *a, item *h, int side) {
...
extern void simpreduce(int side, item **f, item **l, allinfo *a) {
    register item *i, *j, *k;
...
extern void reduceset(allinfo *a) {
...
extern void initfirst(allinfo *a, stype ps, stype ws) {
    register state *k;
...
extern void initvect(allinfo *a) {
    register btype i;
...
extern void copyproblem(item *f, item *l, int *p, int *w, int *x) {
    register item *i, *m;
...
extern void findbreak(allinfo *a) {
    register item *i, *m;
...
extern std::tuple<double, double> minknap_cpp(int n, int *p, int *w, int *x,
    ...
    return std::make_tuple(elapsedTime, z);
}
}  // namespace minknap
