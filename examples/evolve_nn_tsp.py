...
from digneapy.transformers.tuner import Tunner
...
def main():
    ...
    cma_es = Tunner(
        dimension=dimension, ranges=(-1.0, 1.0), generations=100, lambda_=32, seed=seed
    )
    solution = cma_es(eval_fn=fitness)
    with open(f"tsp_NN_weights_N_50_2D_{repetition}.npy", "wb") as f:
        np.save(f, np.asarray(solution.x))
    with open(f"tsp_fitness_NN_N_50_2D_{repetition}.npy.npy", "wb") as f:
        np.save(f, np.asarray(solution.fun))

    # Save the model itself
    nn.update_weights(solution.x)
    nn.save(f"TSP_NN_best_transformer_N_120_to_2D_{repetition}.keras")
